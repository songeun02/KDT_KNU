...
def show_html(predict_age):
    ...
    print("""
        <style>
            ...
            #content {text-align: left; margin-left: 10%; width: 80%;}
        </style>
    """)
    print(f"""
    </head>
    <body id = "body">
        ...
        <div id = "section_ml" style="border:5px solid rgb(255,170,170)">
            ...
            <form id ="content" action="/cgi-bin/web_ml.py" method="POST">
                ...
                건조된 살점 무게 (Shell Weight) <input type="text" name="shell_weight"><br><br>
...
def make_array(sex_array, length, diameter, height, whole_weight, shucked_weight, viscera_weight, shell_weight):
    ...
    input_data = np.array([[
        ...
        float(height),
        float(whole_weight),
        float(shucked_weight),
        float(viscera_weight),
        float(shell_weight) 
    ]])
    return input_data
...
# (2) 모델 로딩
if SCRIPT_MODE:
    mlfile = os.path.dirname(__file__)+ '/model_ml_bagging.joblib' # 웹상에서는 절대경로만
else:
    mlfile = '../model_ml_bagging.joblib'
...
if len(length):
    ...
    # 예측하기
    predicted_value = mlModel.predict(input_data)
    predicted_value = predicted_value[0]

# (4) 사용자에게 WEB 화면 제공
show_html(predicted_value)
